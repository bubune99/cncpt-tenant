...
export const PromptInput = ({ className, ...props }: PromptInputProps) => (
  <form
    className={cn(
      "w-full overflow-hidden rounded-2xl border bg-background/80 backdrop-blur-sm shadow-lg",
...
export const PromptInputSubmit = ({
  ...
  let Icon = <SendIcon className="size-4 transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />;

  if (status === "submitted") {
    Icon = <Loader2Icon className="size-4 animate-spin" />;
  } else if (status === "streaming") {
    Icon = <SquareIcon className="size-4 animate-pulse" />;
...
export const PromptInputModelSelectContent = ({
  ...
  <SelectContent className={cn("z-[100]", className)} {...props} />
);
