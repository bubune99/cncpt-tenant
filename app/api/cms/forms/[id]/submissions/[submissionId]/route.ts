/**
 * Individual Submission API (tenant-scoped)
 *
...
// GET — admin-only (single submission — exposes contact details/PII)
export async function GET(request: NextRequest, { params }: RouteParams) {
  return withTenantAuth(request, 'view', async () => {
    try {
      const { id, submissionId } = await params

      // Check if form exists (auto-scoped to tenant by Prisma middleware)
