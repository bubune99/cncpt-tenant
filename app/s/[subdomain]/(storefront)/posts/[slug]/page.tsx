  ...
  return (
    <article className="container mx-auto px-4 py-6 sm:py-8 lg:py-12">
      ...
      <header className="max-w-3xl mx-auto mb-8 sm:mb-12">
        ...
        {/* Title */}
        <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-4 sm:mb-6">{post.title}</h1>

        {/* Excerpt */}
        {post.excerpt && (
          <p className="text-base sm:text-lg lg:text-xl text-muted-foreground mb-4 sm:mb-6">{post.excerpt}</p>
        )}
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {post.tags.map(({ tag }: (typeof post.tags)[number]) => (
                <Link key={tag.id} href={`/tags/${tag.slug}`}>
                  <Badge variant="outline" className="py-1.5">{tag.name}</Badge>
                </Link>
              ))}
            </div>
          </div>
          ...
          <div className="mt-8 sm:mt-12 pt-6 sm:pt-8 border-t">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                ...
                <p className="font-semibold text-sm sm:text-base">{post.author.name || post.author.email}</p>
              </div>
            </div>
          </div>
        )}
