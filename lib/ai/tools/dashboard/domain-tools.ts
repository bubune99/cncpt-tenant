...
export const listDomains = tool({
  ...
  execute: async ({ subdomain, status }, { userId }) => {
    if (!userId) {
    ...
    try {
      ...
      if (subdomain) {
        // Check user owns this subdomain
        ...
        // All domains across user's subdomains
        if (status === "verified") {
          query = sql`
            SELECT cd.*, s.emoji
            FROM custom_domains cd
 ...
 */
export const getDomainStatus = tool({
  description:
  ...
  execute: async ({ domain }, { userId }) => {
    ...
    try {
      ...
      const dnsRecords = getDnsRecordsForDomain(domain, d.subdomain)
 ...
 */
export const troubleshootDomain = tool({
  description:
  ...
  execute: async ({ domain }, { userId }) => {
    ...
    try {
      ...
      // Check if it's a www subdomain without the root
      ...
      // Check if root domain without www
      ...
      return {
        ...
        isPrimary: d.is_primary,
        issues,
        dnsRecords: getDnsRecordsForDomain(domain, d.subdomain),
      }
    } catch (error) {
 ...
 */
export const getDnsInstructions = tool({
  description:
  ...
  execute: async ({ domain, provider }) => {
    const subdomain = "your-subdomain" // Generic placeholder
...
function getDnsRecordsForDomain(domain: string, subdomain: string) {
  ...
  // For apex domains, use ALIAS/ANAME or A record
