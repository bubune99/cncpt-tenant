...
/**
 * Create team tools with user context
 */
export function createTeamTools(userId: string) {
  /**
    ...
    execute: async ({ teamId }: { teamId: string }) => {
      ...
      try {
        ...
        const roleCount = members.reduce(
          (acc, m) => {
            acc[m.role as string] = (acc[m.role as string] || 0) + 1
            return acc
          },
    ...
    execute: async ({ teamId }: { teamId: string }) => {
      ...
      try {
        ...
        const accessLevelCount = subdomains.reduce(
          (acc, s) => {
            acc[s.access_level as string] = (acc[s.access_level as string] || 0) + 1
            return acc
          },
          {} as Record<string, number>
        )
