...
async function getAnalyticsSettings() {
  ...
  const settings = __spreadValues({}, DEFAULT_ANALYTICS_SETTINGS);
...
async function ensureUniqueSlug(slug, type, excludeId) {
  ...
  let counter = 1;
  while (true) {
    const existing = await (type === "post" ? prisma.blogPost.findUnique({ where: { slug: uniqueSlug } }) : type === "category" ? prisma.blogCategory.findUnique({ where: { slug: uniqueSlug } }) : prisma.blogTag.findUnique({ where: { slug: uniqueSlug } }));
...
async function getCartStats() {
  const [active, abandoned, recovered, converted, avgValue] = await Promise.all([
    prisma.cart.count({ where: { status: import_client.CartStatus.ACTIVE } }),
    prisma.cart.count({ where: { status: import_client.CartStatus.ABANDONED } }),
    prisma.cart.count({ where: { recoveredAt: { not: null } } }),
    prisma.cart.count({ where: { status: import_client.CartStatus.CONVERTED } }),
...
async function getStripeSettings() {
  ...
  const settings = __spreadValues({}, DEFAULT_STRIPE_SETTINGS);
...
async function createCheckoutSession(request) {
  ...
  const sessionParams = {
    ...
    metadata: __spreadProps(__spreadValues({}, request.metadata), {
...
async function getStripeClient2() {
  ...
  if (!stripeClient2) {
    stripeClient2 = new Stripe2(secretKey, { apiVersion: "2025-02-24.acacia" });
  }
...
async function createMedia(input) {
  const _a = input, { tagIds } = _a, data = __objRest(_a, ["tagIds"]);
...
function renderTemplate(template, data, options = {}) {
  ...
  const mergedData = __spreadProps(__spreadValues({}, data), {
    store: __spreadValues(__spreadValues(__spreadValues({}, defaultStoreConfig), store), dataStore),
...
function renderEmail(content, templateOptions, data, renderOptions = {}) {
  ...
  const wrappedHtml = wrapInBaseTemplate(content, __spreadProps(__spreadValues({}, templateOptions), {
    store
  }));
...
async function renderEmailTemplateData(template, data) {
  const emailSettings = await getEmailSettings();
  const mergeData = __spreadProps(__spreadValues({}, flattenData(data)), {
    store: {
  ...
  html = wrapInBaseTemplate(html, {
    title: subject,
    preheader: template.preheader ? parseMergeTags(template.preheader, mergeData) : void 0,
    store: {
      name: emailSettings.fromName || "Our Store",
      url: process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000",
  ...
  switch (component.type) {
    ...
    case "Table":
      ...
      const tableRows = rows.map((row) => {
        const cells = Object.entries(row).map(([key, value]) => `<td style="padding: 8px; border-bottom: 1px solid #eee;">${parseMergeTags(String(value), mergeData)}</td>`).join("");
        return `<tr>${cells}</tr>`;
      }).join("");
...
async function createWorkflow(input) {
  const _a = input, { stages } = _a, workflowData = __objRest(_a, ["stages"]);
  ...
  const workflow = await prisma.orderWorkflow.create({
    data: __spreadProps(__spreadValues({}, workflowData), {
      stages: stages ? {
        create: stages.map((stage, index) => {
          var _a2;
          return __spreadProps(__spreadValues({}, stage), {
...
function createSandboxFunction(code, config = {}) {
  const { allowAsync: _allowAsync } = __spreadValues(__spreadValues({}, DEFAULT_CONFIG), config);
  const wrappedCode = `
    "use strict";
    ${code}
  `;
...
async function executeSandbox(code, args, context, config = {}) {
  const fullConfig = __spreadValues(__spreadValues({}, DEFAULT_CONFIG), config);
...
var PluginRegistry = class {
  ...
  async getPrimitiveFromDb(id) {
    const db2 = await prisma.primitive.findUnique({ where: { id } });
    if (!db2) return null;
...
async function executePrimitive(primitive, args, context = {}, options = {}) {
  ...
  const execContext = __spreadValues({
...
async function getReviews(filters = {}, sort = { field: "createdAt", direction: "desc" }, page = 1, pageSize = 10) {
  ...
  if (filters.minRating) {
    where.rating = __spreadProps(__spreadValues({}, where.rating), { gte: filters.minRating });
  }
  if (filters.maxRating) {
    where.rating = __spreadProps(__spreadValues({}, where.rating), { lte: filters.maxRating });
  }
...
async function getSeoConfig() {
  ...
  const config = __spreadValues({}, DEFAULT_SEO_CONFIG);
...
var getSiteSettings = cache(async () => {
  try {
...
var EventBus = class {
  ...
  subscribe(subscription) {
    const id = uuidv4();
    const fullSubscription = __spreadProps(__spreadValues({}, subscription), { id });
  ...
  async triggerEventWorkflows(event) {
    try {
      ...
      for (const workflow of workflows) {
        ...
        const { executeWorkflow: executeWorkflow2 } = await import("./engine-TJVD7WI5.mjs");
...
registerAction({
  ...
  handler: async (input) => {
    ...
    switch (type) {
      ...
      case "currency":
        return new Intl.NumberFormat(locale, __spreadValues({
          style: "currency",
          currency: "USD"
...
async function getWorkflowTemplates(options) {
  ...
  const templates = await prisma.workflowTemplate.findMany({
    where: __spreadValues(__spreadValues(__spreadValues({}, onlyActive && { isActive: true }), category && { category }), search && {
      OR: [
        { name: { contains: search, mode: "insensitive" } },
        { description: { contains: search, mode: "insensitive" } },
  ...
  sortedSteps.forEach((step, index) => {
    ...
    nodes.push({
      ...
      data: {
        label: step.name,
        stepType: step.type,
        config: __spreadValues(__spreadValues({}, step.config), getConfigOverridesForStep(step.name, configOverrides)),
...
async function getShippingSettings() {
  ...
  try {
    ...
    if (settings.length > 0) {
      const settingsMap = {};
      for (const setting of settings) {
        settingsMap[setting.key] = setting.encrypted ? safeDecrypt(setting.value) : setting.value;
      }
      const dbSettings = {
...
async function getDashboardConfig() {
  try {
    ...
    if (setting == null ? void 0 : setting.value) {
      ...
      return __spreadValues(__spreadValues({}, getDefaultDashboardConfig(config.preset)), config);
    }
  } catch (error) {
...
async function saveDashboardConfig(config) {
  ...
  const newConfig = __spreadValues(__spreadValues({}, currentConfig), config);
...
async function addTab(tab) {
  ...
  config.tabs.push(__spreadProps(__spreadValues({}, tab), {
    order: maxOrder + 1
  }));
  return saveDashboardConfig(config);
}
...
async function reorderTabs(tabIds) {
  ...
  config.tabs = tabIds.map((id, index) => {
    ...
    if (tab) {
      return __spreadProps(__spreadValues({}, tab), { order: index + 1 });
    }
    return null;
  }).filter((t) => t !== null);
  return saveDashboardConfig(config);
...
async function addWidget(tabId, widget) {
  ...
  if (tabIndex >= 0) {
    ...
    config.tabs[tabIndex].widgets.push(__spreadProps(__spreadValues({}, widget), {
      order: maxOrder + 1
    }));
    return saveDashboardConfig(config);
  }
...
var getRouteConfig = cache2(async (slug) => {
...
//# sourceMappingURL=chunk-G5BYVDQS.mjs.map