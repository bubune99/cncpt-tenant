...
async function getAnalyticsSettings() {
  ...
  const records = await _chunkH7VBPMCNjs.prisma.setting.findMany({
...
async function trackServerEvent(eventName, eventData, context) {
  try {
    await _chunkH7VBPMCNjs.prisma.analyticsEvent.create({
      data: {
        eventName,
...
async function getAnalyticsSummary(startDate, endDate) {
  const pageViews = await _chunkH7VBPMCNjs.prisma.analyticsEvent.count({
    where: {
      eventName: "page_view",
      createdAt: { gte: startDate, lte: endDate }
    }
  });
  const uniqueVisitors = await _chunkH7VBPMCNjs.prisma.analyticsEvent.groupBy({
    by: ["sessionId"],
    where: {
      sessionId: { not: null },
      createdAt: { gte: startDate, lte: endDate }
  ...
  const purchases = await _chunkH7VBPMCNjs.prisma.analyticsEvent.findMany({
    where: {
      eventName: "purchase",
      createdAt: { gte: startDate, lte: endDate }
    },
  ...
  const topPagesRaw = await _chunkH7VBPMCNjs.prisma.analyticsEvent.groupBy({
    by: ["pageUrl"],
    where: {
      eventName: "page_view",
      pageUrl: { not: null },
  ...
  const topReferrersRaw = await _chunkH7VBPMCNjs.prisma.analyticsEvent.groupBy({
    by: ["referrer"],
    where: {
      referrer: { not: null },
      createdAt: { gte: startDate, lte: endDate }
  ...
  const eventBreakdownRaw = await _chunkH7VBPMCNjs.prisma.analyticsEvent.groupBy({
    by: ["eventName"],
    where: {
      createdAt: { gte: startDate, lte: endDate }
    },
...
async function ensureUniqueSlug(slug, type, excludeId) {
  ...
  let counter = 1;
  while (true) {
    const existing = await (type === "post" ? _chunkH7VBPMCNjs.prisma.blogPost.findUnique({ where: { slug: uniqueSlug } }) : type === "category" ? _chunkH7VBPMCNjs.prisma.blogCategory.findUnique({ where: { slug: uniqueSlug } }) : _chunkH7VBPMCNjs.prisma.blogTag.findUnique({ where: { slug: uniqueSlug } }));
...
async function createPost(input) {
  ...
  const post = await _chunkH7VBPMCNjs.prisma.blogPost.create({
    data: {
      title: input.title,
      slug: uniqueSlug,
      ...
      puckContent: input.puckContent,
      usePuckLayout: (_a = input.usePuckLayout) != null ? _a : false,
      authorId: input.authorId,
      featuredImageId: input.featuredImageId,
      status: (_b = input.status) != null ? _b : "DRAFT",
      visibility: (_c = input.visibility) != null ? _c : "PUBLIC",
      metaTitle: input.metaTitle,
      metaDescription: input.metaDescription,
      canonicalUrl: input.canonicalUrl,
      noIndex: (_d = input.noIndex) != null ? _d : false,
      ogTitle: input.ogTitle,
      ogDescription: input.ogDescription,
      ogImageId: input.ogImageId,
      publishedAt: input.status === "PUBLISHED" ? (_e = input.publishedAt) != null ? _e : /* @__PURE__ */ new Date() : input.publishedAt,
      scheduledAt: input.scheduledAt,
      wordCount,
      readingTime,
      allowComments: (_f = input.allowComments) != null ? _f : true,
      featured: (_g = input.featured) != null ? _g : false,
      pinned: (_h = input.pinned) != null ? _h : false,
      categories: ((_i = input.categoryIds) == null ? void 0 : _i.length) ? {
        create: input.categoryIds.map((categoryId) => ({
          category: { connect: { id: categoryId } }
        }))
      } : void 0,
      tags: ((_j = input.tagIds) == null ? void 0 : _j.length) ? {
...
async function getPost(id) {
  return _chunkH7VBPMCNjs.prisma.blogPost.findUnique({
    where: { id },
    include: {
      author: {
        select: { id: true, name: true, email: true }
...
async function listPosts(options = {}) {
  ...
  const [posts, total] = await Promise.all([
    _chunkH7VBPMCNjs.prisma.blogPost.findMany({
      where,
      take: limit,
      skip: offset,
    ...
    _chunkH7VBPMCNjs.prisma.blogPost.count({ where })
  ]);
  return { posts, total, limit, offset };
}
async function updatePost(id, input) {
  var _a, _b;
  const existing = await _chunkH7VBPMCNjs.prisma.blogPost.findUnique({
    where: { id },
    include: {
      categories: true,
      tags: true
  ...
  if (input.categoryIds !== void 0) {
    await _chunkH7VBPMCNjs.prisma.blogPostCategory.deleteMany({
      where: { postId: id }
    });
  }
  if (input.tagIds !== void 0) {
    await _chunkH7VBPMCNjs.prisma.blogPostTag.deleteMany({
      where: { postId: id }
    });
  }
  const post = await _chunkH7VBPMCNjs.prisma.blogPost.update({
    where: { id },
    data: {
      title: input.title,
      slug,
...
async function deletePost(id) {
  await _chunkH7VBPMCNjs.prisma.blogPostCategory.deleteMany({ where: { postId: id } });
  await _chunkH7VBPMCNjs.prisma.blogPostTag.deleteMany({ where: { postId: id } });
  await _chunkH7VBPMCNjs.prisma.blogComment.deleteMany({ where: { postId: id } });
  return _chunkH7VBPMCNjs.prisma.blogPost.delete({ where: { id } });
}
async function incrementPostViews(id) {
  return _chunkH7VBPMCNjs.prisma.blogPost.update({
    where: { id },
...
async function createCategory(input) {
  ...
  return _chunkH7VBPMCNjs.prisma.blogCategory.create({
    data: {
      name: input.name,
      slug: uniqueSlug,
...
async function getCategory(id) {
  return _chunkH7VBPMCNjs.prisma.blogCategory.findUnique({
    where: { id },
    include: {
      parent: true,
      children: true,
...
async function listCategories(options = {}) {
  ...
  const [categories, total] = await Promise.all([
    _chunkH7VBPMCNjs.prisma.blogCategory.findMany({
      where,
      take: limit,
      skip: offset,
      orderBy: { name: "asc" },
    ...
    _chunkH7VBPMCNjs.prisma.blogCategory.count({ where })
  ]);
  return { categories, total, limit, offset };
}
async function updateCategory(id, input) {
  const existing = await _chunkH7VBPMCNjs.prisma.blogCategory.findUnique({ where: { id } });
  if (!existing) {
  ...
  return _chunkH7VBPMCNjs.prisma.blogCategory.update({
    where: { id },
    data: {
      name: input.name,
      slug,
...
async function deleteCategory(id) {
  const children = await _chunkH7VBPMCNjs.prisma.blogCategory.count({ where: { parentId: id } });
  if (children > 0) {
    throw new Error("Cannot delete category with child categories");
  }
  await _chunkH7VBPMCNjs.prisma.blogPostCategory.deleteMany({ where: { categoryId: id } });
  return _chunkH7VBPMCNjs.prisma.blogCategory.delete({ where: { id } });
}
async function createTag(input) {
  const slug = input.slug || generateSlug(input.name);
  const uniqueSlug = await ensureUniqueSlug(slug, "tag");
  return _chunkH7VBPMCNjs.prisma.blogTag.create({
    data: {
      name: input.name,
      slug: uniqueSlug,
      description: input.description
...
async function getTag(id) {
  return _chunkH7VBPMCNjs.prisma.blogTag.findUnique({
    where: { id },
    include: {
      _count: {
        select: { posts: true }
...
async function listTags(options = {}) {
  ...
  const [tags, total] = await Promise.all([
    _chunkH7VBPMCNjs.prisma.blogTag.findMany({
      where,
      take: limit,
      skip: offset,
      orderBy: { name: "asc" },
    ...
    _chunkH7VBPMCNjs.prisma.blogTag.count({ where })
  ]);
  return { tags, total, limit, offset };
}
async function updateTag(id, input) {
  const existing = await _chunkH7VBPMCNjs.prisma.blogTag.findUnique({ where: { id } });
  if (!existing) {
  ...
  return _chunkH7VBPMCNjs.prisma.blogTag.update({
    where: { id },
    data: {
      name: input.name,
      slug,
...
async function deleteTag(id) {
  await _chunkH7VBPMCNjs.prisma.blogPostTag.deleteMany({ where: { tagId: id } });
  return _chunkH7VBPMCNjs.prisma.blogTag.delete({ where: { id } });
}

// src/lib/cart/index.ts
var import_client = _chunkHY7GTCJMjs.__toESM.call(void 0, _chunkH7VBPMCNjs.require_default.call(void 0, ));
async function getOrCreateCart(identifier) {
  const { sessionId, userId, cartId } = identifier;
  if (cartId) {
    const cart2 = await _chunkH7VBPMCNjs.prisma.cart.findUnique({
      where: { id: cartId },
      include: {
        items: {
  ...
  if (!cart) {
    cart = await _chunkH7VBPMCNjs.prisma.cart.create({
      data: {
...
async function findExistingCart(identifier) {
  ...
  if (userId) {
    const cart = await _chunkH7VBPMCNjs.prisma.cart.findFirst({
      where: {
        userId,
        status: import_client.CartStatus.ACTIVE
      },
  ...
  if (sessionId) {
    const cart = await _chunkH7VBPMCNjs.prisma.cart.findUnique({
      where: { sessionId },
      include: {
        items: {
          orderBy: { createdAt: "asc" }
...
async function addToCart(cartId, item) {
  ...
  const product = await _chunkH7VBPMCNjs.prisma.product.findUnique({
    where: { id: productId },
    include: {
  ...
  const variant = variantId ? await _chunkH7VBPMCNjs.prisma.productVariant.findUnique({ where: { id: variantId } }) : null;
  ...
  const existingItem = await _chunkH7VBPMCNjs.prisma.cartItem.findUnique({
    where: {
  ...
  if (existingItem) {
    await _chunkH7VBPMCNjs.prisma.cartItem.update({
      where: { id: existingItem.id },
      data: { quantity: existingItem.quantity + quantity }
    });
  } else {
    await _chunkH7VBPMCNjs.prisma.cartItem.create({
      data: {
        cartId,
        productId,
        variantId,
...
async function removeFromCart(cartId, itemId) {
  await _chunkH7VBPMCNjs.prisma.cartItem.delete({
    where: { id: itemId, cartId }
  });
  return recalculateCart(cartId);
}
async function clearCart(cartId) {
  await _chunkH7VBPMCNjs.prisma.cartItem.deleteMany({
    where: { cartId }
  });
  return recalculateCart(cartId);
}
async function applyDiscount(cartId, code) {
  const discount = await _chunkH7VBPMCNjs.prisma.discountCode.findUnique({
  ...
  await _chunkH7VBPMCNjs.prisma.cart.update({
    where: { id: cartId },
...
async function mergeCartsOnLogin(sessionId, userId) {
  const guestCart = await _chunkH7VBPMCNjs.prisma.cart.findUnique({
    where: { sessionId },
    include: { items: true }
  });
  ...
  let userCart = await _chunkH7VBPMCNjs.prisma.cart.findFirst({
...
async function markAbandonedCarts(timeoutMinutes = 60) {
  ...
  const result = await _chunkH7VBPMCNjs.prisma.cart.updateMany({
    where: {
      status: import_client.CartStatus.ACTIVE,
...
async function getAbandonedCartsForRecovery(minAgeMinutes = 60, maxAgeHours = 72) {
  ...
  const carts = await _chunkH7VBPMCNjs.prisma.cart.findMany({
    where: {
      status: import_client.CartStatus.ABANDONED,
      abandonedAt: {
        gte: maxAge,
...
async function cleanupExpiredCarts(expiryDays = 30) {
  ...
  const result = await _chunkH7VBPMCNjs.prisma.cart.deleteMany({
    where: {
      OR: [
        { status: import_client.CartStatus.EXPIRED },
        {
...
async function getCartStats() {
  const [active, abandoned, recovered, converted, avgValue] = await Promise.all([
    _chunkH7VBPMCNjs.prisma.cart.count({ where: { status: import_client.CartStatus.ACTIVE } }),
    _chunkH7VBPMCNjs.prisma.cart.count({ where: { status: import_client.CartStatus.ABANDONED } }),
    _chunkH7VBPMCNjs.prisma.cart.count({ where: { recoveredAt: { not: null } } }),
    _chunkH7VBPMCNjs.prisma.cart.count({ where: { status: import_client.CartStatus.CONVERTED } }),
    _chunkH7VBPMCNjs.prisma.cart.aggregate({
...
async function validateDiscountCode(context) {
  ...
  if (discount.perCustomer !== null && (userId || email)) {
    const customerUsageCount = await _chunkH7VBPMCNjs.prisma.discountUsage.count({
      where: {
...
async function isFirstOrderForUser(userId, email) {
  ...
  const existingOrders = await _chunkH7VBPMCNjs.prisma.order.count({
    where: {
      OR: [
...
async function recordDiscountUsage(discountCodeId, orderId, userId, email, discountAmount) {
  const [usage] = await _chunkH7VBPMCNjs.prisma.$transaction([
    _chunkH7VBPMCNjs.prisma.discountUsage.create({
      data: {
        discountCodeId,
        orderId,
        userId,
    ...
    _chunkH7VBPMCNjs.prisma.discountCode.update({
...
async function getStripeSettings() {
  ...
  const settingRecords = await _chunkH7VBPMCNjs.prisma.setting.findMany({
    where: {
...
async function createStripeCoupon(discountId) {
  ...
  await _chunkH7VBPMCNjs.prisma.discountCode.update({
    where: { id: discountId },
    data: {
...
async function importFromStripe(promotionCodeId) {
  ...
  const existing = await _chunkH7VBPMCNjs.prisma.discountCode.findFirst({
    where: {
      OR: [
  ...
  const discount = await _chunkH7VBPMCNjs.prisma.discountCode.create({
    data: {
      code: promotionCode.code,
...
async function listUnimportedStripeCoupons() {
  ...
  const importedCoupons = await _chunkH7VBPMCNjs.prisma.discountCode.findMany({
...
async function createForm(name, fields, settings) {
  ...
  const form = await _chunkH7VBPMCNjs.prisma.form.create({
    data: {
      name,
      slug,
      fields,
...
async function getForm(idOrSlug) {
  const form = await _chunkH7VBPMCNjs.prisma.form.findFirst({
    where: {
...
async function updateForm(id, data) {
  ...
  const form = await _chunkH7VBPMCNjs.prisma.form.update({
    where: { id },
    data: updateData
  });
  return {
...
async function deleteForm(id) {
  await _chunkH7VBPMCNjs.prisma.form.delete({ where: { id } });
}
...
async function submitForm(submission) {
  ...
  const formSubmission = await _chunkH7VBPMCNjs.prisma.formSubmission.create({
    data: {
      formId: form.id,
  ...
  await _chunkH7VBPMCNjs.prisma.form.update({
    where: { id: form.id },
...
async function getFormSubmissions(formId, options) {
  ...
  const [submissions, total] = await Promise.all([
    _chunkH7VBPMCNjs.prisma.formSubmission.findMany({
      where,
      take: (options == null ? void 0 : options.limit) || 50,
      skip: (options == null ? void 0 : options.offset) || 0,
      orderBy: { createdAt: "desc" }
    }),
    _chunkH7VBPMCNjs.prisma.formSubmission.count({ where })
  ]);
  return {
...
async function markSubmissionRead(submissionId, read = true) {
  await _chunkH7VBPMCNjs.prisma.formSubmission.update({
...
async function deleteSubmission(submissionId) {
  await _chunkH7VBPMCNjs.prisma.formSubmission.delete({
    where: { id: submissionId }
  });
}
...
async function getLowStockItems() {
  const lowStockItems = [];
  const lowStockProducts = await _chunkH7VBPMCNjs.prisma.product.findMany({
    where: {
      status: "ACTIVE",
      trackInventory: true,
      type: "SIMPLE",
      stock: {
        lte: _chunkH7VBPMCNjs.prisma.product.fields.lowStockThreshold
      }
    },
    select: {
      id: true,
  ...
  const lowStockVariants = await _chunkH7VBPMCNjs.prisma.productVariant.findMany({
    where: {
      enabled: true,
      product: {
        status: "ACTIVE",
...
async function sendLowStockAlert(items) {
  ...
  const emailSettings = await _chunkZNGPT5BQjs.getEmailSettings.call(void 0, );
...
View products: ${appUrl}/admin/products`;
  try {
    const result = await _chunkZNGPT5BQjs.sendEmail.call(void 0, {
...
async function subscribeToBackInStock(email, productId, variantId) {
  try {
    ...
    if (variantId) {
      const variant = await _chunkH7VBPMCNjs.prisma.productVariant.findUnique({
        where: { id: variantId },
        select: { id: true }
      });
      if (!variant) {
    ...
    const existing = await _chunkH7VBPMCNjs.prisma.backInStockSubscription.findFirst({
      where: {
        email,
        productId,
    ...
    if (existing) {
      await _chunkH7VBPMCNjs.prisma.backInStockSubscription.update({
      ...
      await _chunkH7VBPMCNjs.prisma.backInStockSubscription.create({
        data: {
          email,
          productId,
          variantId: variantId != null ? variantId : null
...
async function unsubscribeFromBackInStock(email, productId, variantId) {
  try {
    await _chunkH7VBPMCNjs.prisma.backInStockSubscription.deleteMany({
      where: {
        email,
        productId,
...
async function sendBackInStockNotifications(productId, variantId) {
  var _a, _b, _c, _d;
  const subscriptions = await _chunkH7VBPMCNjs.prisma.backInStockSubscription.findMany({
    where: {
      productId,
      variantId: variantId || null,
      notified: false
...
async function checkAndNotifyBackInStock(productId, variantId, newStock) {
  ...
  const subCount = await _chunkH7VBPMCNjs.prisma.backInStockSubscription.count({
    where: {
      productId,
      variantId: variantId || null,
      notified: false
...
async function reserveStock(productId, quantity, sessionId, variantId, reservationMinutes = DEFAULT_RESERVATION_MINUTES) {
  try {
    ...
    const reservedStock = await _chunkH7VBPMCNjs.prisma.stockReservation.aggregate({
      where: {
        productId,
        variantId: variantId || null,
        released: false,
    ...
    const reservation = await _chunkH7VBPMCNjs.prisma.stockReservation.create({
      data: {
        productId,
        variantId: variantId || null,
        quantity,
...
async function releaseReservation(reservationId) {
  try {
    await _chunkH7VBPMCNjs.prisma.stockReservation.update({
...
async function releaseSessionReservations(sessionId) {
  const result = await _chunkH7VBPMCNjs.prisma.stockReservation.updateMany({
    where: {
      sessionId,
      released: false
    },
...
async function deductStockForOrder(orderId) {
  try {
    const reservations = await _chunkH7VBPMCNjs.prisma.stockReservation.findMany({
      where: {
        orderId,
        released: false
      }
    ...
    for (const reservation of reservations) {
      if (reservation.variantId) {
        await _chunkH7VBPMCNjs.prisma.productVariant.update({
        ...
        await _chunkH7VBPMCNjs.prisma.product.update({
...
async function getAvailableStock(productId, variantId) {
  ...
  const reserved = await _chunkH7VBPMCNjs.prisma.stockReservation.aggregate({
    where: {
      productId,
      variantId: variantId || null,
      released: false,
...
var db = _chunkH7VBPMCNjs.prisma;
...
function renderTemplate(template, data, options = {}) {
  ...
  return _chunkZNGPT5BQjs.parseMergeTags.call(void 0, template, sanitizedData);
}
...
async function renderEmailTemplateBySlug(slug, data) {
  const template = await _chunkH7VBPMCNjs.prisma.emailTemplate.findUnique({
    where: { slug }
  });
  if (!template) {
    return null;
...
async function renderEmailTemplateData(template, data) {
  ...
  subject = _chunkZNGPT5BQjs.parseMergeTags.call(void 0, subject, mergeData);
  ...
  html = _chunkZNGPT5BQjs.parseMergeTags.call(void 0, html, mergeData);
  html = wrapInBaseTemplate(html, {
    title: subject,
    preheader: template.preheader ? _chunkZNGPT5BQjs.parseMergeTags.call(void 0, template.preheader, mergeData) : void 0,
    store: {
      name: emailSettings.fromName || "Our Store",
      url: process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000",
...
function renderPuckComponent(component, mergeData) {
  ...
  switch (component.type) {
    case "Heading":
      ...
      const headingText = _chunkZNGPT5BQjs.parseMergeTags.call(void 0, String(props.text || ""), mergeData);
      return `<${level} style="color: #333; margin: 0 0 16px 0;">${headingText}</${level}>`;
    case "Text":
    case "Paragraph":
      const text = _chunkZNGPT5BQjs.parseMergeTags.call(void 0, String(props.text || props.content || ""), mergeData);
      return `<p style="color: #333; line-height: 1.6; margin: 0 0 16px 0;">${text}</p>`;
    case "Button":
      const buttonText = _chunkZNGPT5BQjs.parseMergeTags.call(void 0, String(props.label || props.text || "Click Here"), mergeData);
      const buttonUrl = _chunkZNGPT5BQjs.parseMergeTags.call(void 0, String(props.url || props.href || "#"), mergeData);
      ...
      `;
    case "Image":
      const src = _chunkZNGPT5BQjs.parseMergeTags.call(void 0, String(props.src || props.url || ""), mergeData);
      const alt = _chunkZNGPT5BQjs.parseMergeTags.call(void 0, String(props.alt || ""), mergeData);
    ...
    case "Table":
      ...
      const tableRows = rows.map((row) => {
        const cells = Object.entries(row).map(([key, value]) => `<td style="padding: 8px; border-bottom: 1px solid #eee;">${_chunkZNGPT5BQjs.parseMergeTags.call(void 0, String(value), mergeData)}</td>`).join("");
        return `<tr>${cells}</tr>`;
      }).join("");
    ...
    default:
      if (props.children && typeof props.children === "string") {
        return `<div>${_chunkZNGPT5BQjs.parseMergeTags.call(void 0, props.children, mergeData)}</div>`;
      }
      return "";
  }
}
...
async function getStoreConfig() {
  const settings = await _chunkZNGPT5BQjs.getEmailSettings.call(void 0, );
  return {
...
async function sendOrderConfirmation(orderId) {
  var _a;
  try {
    const order = await _chunkH7VBPMCNjs.prisma.order.findUnique({
      where: { id: orderId },
      include: {
        items: {
          include: {
...
async function sendShippingNotification(orderId, shipmentId) {
  ...
  try {
    ...
    const shipment = await _chunkH7VBPMCNjs.prisma.shipment.findUnique({
      where: { id: shipmentId }
    });
    if (!shipment) {
...
async function getUserPermissions(userId) {
  const user = await _chunkH7VBPMCNjs.prisma.user.findUnique({
    where: { id: userId },
    include: {
      roleAssignments: {
        include: {
...
async function logAuditEvent(params) {
  await _chunkH7VBPMCNjs.prisma.auditLog.create({
    data: {
...
async function seedBuiltInRoles() {
  for (const [, roleData] of Object.entries(BUILT_IN_ROLES)) {
    await _chunkH7VBPMCNjs.prisma.role.upsert({
...
async function assignRole(params) {
  await _chunkH7VBPMCNjs.prisma.roleAssignment.create({
    data: {
      userId: params.userId,
  ...
  if (params.assignedBy) {
    const assigner = await _chunkH7VBPMCNjs.prisma.user.findUnique({
      where: { id: params.assignedBy },
      select: { email: true }
    });
    const role = await _chunkH7VBPMCNjs.prisma.role.findUnique({
...
async function removeRole(params) {
  await _chunkH7VBPMCNjs.prisma.roleAssignment.delete({
    where: {
      userId_roleId: {
        userId: params.userId,
        roleId: params.roleId
  ...
  if (params.removedBy) {
    const remover = await _chunkH7VBPMCNjs.prisma.user.findUnique({
...
async function grantPermission(params) {
  var _a;
  await _chunkH7VBPMCNjs.prisma.userPermission.upsert({
    where: {
      userId_permission: {
        userId: params.userId,
  ...
  if (params.grantedBy) {
    const granter = await _chunkH7VBPMCNjs.prisma.user.findUnique({
...
async function denyPermission(params) {
  ...
  if (params.deniedBy) {
    const denier = await _chunkH7VBPMCNjs.prisma.user.findUnique({
...
async function removePermissionOverride(params) {
  const existing = await _chunkH7VBPMCNjs.prisma.userPermission.findUnique({
    where: {
      userId_permission: {
        userId: params.userId,
        permission: params.permission
  ...
  if (!existing) return;
  await _chunkH7VBPMCNjs.prisma.userPermission.delete({
    where: {
      userId_permission: {
        userId: params.userId,
        permission: params.permission
...
async function listWorkflows(includeInactive = false) {
  const workflows = await _chunkH7VBPMCNjs.prisma.orderWorkflow.findMany({
...
async function getWorkflow(idOrSlug) {
  const workflow = await _chunkH7VBPMCNjs.prisma.orderWorkflow.findFirst({
    where: {
      OR: [{ id: idOrSlug }, { slug: idOrSlug }]
    },
    include: {
...
async function createWorkflow(input) {
  ...
  if (workflowData.isDefault) {
    await _chunkH7VBPMCNjs.prisma.orderWorkflow.updateMany({
      where: { isDefault: true },
      data: { isDefault: false }
    });
  }
  const workflow = await _chunkH7VBPMCNjs.prisma.orderWorkflow.create({
...
async function updateWorkflow(id, input) {
  ...
  const workflow = await _chunkH7VBPMCNjs.prisma.orderWorkflow.update({
    where: { id },
    data: input,
    include: {
      stages: {
...
async function deleteWorkflow(id) {
  const orderCount = await _chunkH7VBPMCNjs.prisma.order.count({
    where: { workflowId: id }
  });
  if (orderCount > 0) {
    throw new Error(`Cannot delete workflow: ${orderCount} orders are using it`);
  }
  await _chunkH7VBPMCNjs.prisma.orderWorkflow.delete({
    where: { id }
  });
}
...
async function determineOrderWorkflow(orderId) {
  const orderItems = await _chunkH7VBPMCNjs.prisma.orderItem.findMany({
    where: { orderId },
    include: {
      product: {
        include: {
...
async function assignWorkflowToOrder(orderId, workflowId) {
  ...
  await _chunkH7VBPMCNjs.prisma.order.update({
    where: { id: orderId },
    data: {
      workflowId,
...
async function initializeOrderWorkflow(orderId) {
  ...
  if (workflowId) {
    ...
    if (order == null ? void 0 : order.currentStageId) {
      await _chunkH7VBPMCNjs.prisma.orderProgress.create({
        data: {
          orderId,
...
var PluginRegistry = class {
  ...
  async createPrimitive(request) {
    ...
    const existing = await _chunkH7VBPMCNjs.prisma.primitive.findUnique({ where: { name: request.name } });
    if (existing) {
    ...
    try {
      const dbPrimitive = await _chunkH7VBPMCNjs.prisma.primitive.create({
        data: {
          id,
          name: request.name,
  ...
  async updatePrimitive(id, request) {
    ...
    try {
      const updated = await _chunkH7VBPMCNjs.prisma.primitive.update({
        where: { id },
        data: {
  ...
  async deletePrimitive(id, force = false) {
    ...
    try {
      await _chunkH7VBPMCNjs.prisma.primitive.delete({ where: { id } });
  ...
  async listPrimitives(options = {}) {
    ...
    if (filter === "mounted") {
      ...
      const dbPrimitives = await _chunkH7VBPMCNjs.prisma.primitive.findMany({ where });
  ...
  async createPlugin(request) {
    ...
    const existing = await _chunkH7VBPMCNjs.prisma.plugin.findFirst({
    ...
    try {
      await _chunkH7VBPMCNjs.prisma.plugin.create({
        data: {
          id,
          name: request.name,
          slug,
  ...
  async setPluginEnabled(id, enabled) {
    try {
      const plugin = await _chunkH7VBPMCNjs.prisma.plugin.findUnique({ where: { id } });
      if (!plugin) {
        return { success: false, error: `Plugin not found: ${id}` };
      }
      await _chunkH7VBPMCNjs.prisma.plugin.update({
        where: { id },
        data: { enabled }
      });
      const primitives = await _chunkH7VBPMCNjs.prisma.primitive.findMany({
  ...
  async deletePlugin(id) {
    try {
      const primitives = await _chunkH7VBPMCNjs.prisma.primitive.findMany({ where: { pluginId: id } });
      for (const prim of primitives) {
        this.dismountPrimitive(prim.id);
        this.primitiveCache.delete(prim.id);
      }
      await _chunkH7VBPMCNjs.prisma.plugin.delete({ where: { id } });
      return { success: true };
    } catch (e) {
      return { success: false, error: `Database error: ${e instanceof Error ? e.message : String(e)}` };
    }
  ...
  async listPlugins(options = {}) {
    ...
    const plugins = await _chunkH7VBPMCNjs.prisma.plugin.findMany({
      where,
  ...
  async getWorkflow(id) {
    const workflow = await _chunkH7VBPMCNjs.prisma.workflow.findUnique({ where: { id } });
  ...
  async listWorkflows(options = {}) {
    ...
    const workflows = await _chunkH7VBPMCNjs.prisma.workflow.findMany({ where });
  ...
  async getStats() {
    const [primitiveCount, pluginStats, workflowCount, executionCount] = await Promise.all([
      _chunkH7VBPMCNjs.prisma.primitive.count(),
      _chunkH7VBPMCNjs.prisma.plugin.aggregate({
        _count: true,
        where: { enabled: true }
      }),
      _chunkH7VBPMCNjs.prisma.workflow.count(),
      _chunkH7VBPMCNjs.prisma.primitiveExecution.count()
    ]);
    return {
      primitiveCount,
      mountedCount: this.mountedPrimitives.size,
      pluginCount: await _chunkH7VBPMCNjs.prisma.plugin.count(),
  ...
  async getPrimitiveFromDb(id) {
    const db2 = await _chunkH7VBPMCNjs.prisma.primitive.findUnique({ where: { id } });
    if (!db2) return null;
...
async function recordExecution(primitiveId, input, output, success, error, startedAt, completedAt, context) {
  try {
    await _chunkH7VBPMCNjs.prisma.primitiveExecution.create({
      data: {
        primitiveId,
...
async function getExecutionStats(primitiveId) {
  const stats = await _chunkH7VBPMCNjs.prisma.primitiveExecution.aggregate({
    where: { primitiveId },
    _count: true,
    _avg: { executionTime: true }
  });
  const successCount = await _chunkH7VBPMCNjs.prisma.primitiveExecution.count({
    where: { primitiveId, success: true }
  });
  const lastExecution = await _chunkH7VBPMCNjs.prisma.primitiveExecution.findFirst({
    where: { primitiveId },
...
async function getRecentExecutions(primitiveId, limit = 10) {
  const executions = await _chunkH7VBPMCNjs.prisma.primitiveExecution.findMany({
    where: { primitiveId },
    orderBy: { startedAt: "desc" },
    take: limit,
    select: {
...
async function loadBuiltInPrimitives() {
  ...
  for (const primitive of BUILT_IN_PRIMITIVES) {
    try {
      const existing = await _chunkH7VBPMCNjs.prisma.primitive.findUnique({
      ...
      await _chunkH7VBPMCNjs.prisma.primitive.upsert({
        where: { name: primitive.name },
        create: {
          name: primitive.name,
...
async function getReviewWithVotes(reviewId) {
  return _chunkH7VBPMCNjs.prisma.productReview.findUnique({
    where: { id: reviewId },
    include: {
      product: {
        select: {
...
async function getReviews(filters = {}, sort = { field: "createdAt", direction: "desc" }, page = 1, pageSize = 10) {
  ...
  const [reviews, total] = await Promise.all([
    _chunkH7VBPMCNjs.prisma.productReview.findMany({
      where,
    ...
    _chunkH7VBPMCNjs.prisma.productReview.count({ where })
  ]);
  return {
    reviews,
    total,
...
async function submitReview(data) {
  ...
  if (data.orderId) {
    const orderItem = await _chunkH7VBPMCNjs.prisma.orderItem.findFirst({
      where: {
        order: {
          id: data.orderId,
          paymentStatus: "PAID"
  ...
  const existingReview = await _chunkH7VBPMCNjs.prisma.productReview.findFirst({
    where: {
      productId: data.productId,
      OR: [
  ...
  const review = await _chunkH7VBPMCNjs.prisma.productReview.create({
    data: {
      productId: data.productId,
      customerId: data.customerId,
...
async function updateReview(reviewId, data) {
  ...
  return _chunkH7VBPMCNjs.prisma.productReview.update({
    where: { id: reviewId },
    data: updateData,
    include: {
      product: {
...
async function deleteReview(reviewId) {
  return _chunkH7VBPMCNjs.prisma.productReview.delete({
    where: { id: reviewId }
  });
}
...
async function voteReview(reviewId, helpful, userId, email) {
  ...
  const existingVote = await _chunkH7VBPMCNjs.prisma.reviewVote.findFirst({
    where: {
      reviewId,
      OR: [
        ...userId ? [{ userId }] : [],
  ...
  if (existingVote) {
    if (existingVote.helpful !== helpful) {
      await _chunkH7VBPMCNjs.prisma.$transaction([
        _chunkH7VBPMCNjs.prisma.reviewVote.update({
          where: { id: existingVote.id },
          data: { helpful }
        }),
        _chunkH7VBPMCNjs.prisma.productReview.update({
          where: { id: reviewId },
          data: {
  ...
  await _chunkH7VBPMCNjs.prisma.$transaction([
    _chunkH7VBPMCNjs.prisma.reviewVote.create({
      data: {
        reviewId,
        userId,
        email,
...
async function getProductReviewStats(productId) {
  const reviews = await _chunkH7VBPMCNjs.prisma.productReview.findMany({
    where: {
      productId,
      status: "APPROVED"
    },
...
async function getReviewDashboardStats() {
  const [total, pending, approved, rejected, flagged, avgRating] = await Promise.all([
    _chunkH7VBPMCNjs.prisma.productReview.count(),
    _chunkH7VBPMCNjs.prisma.productReview.count({ where: { status: "PENDING" } }),
    _chunkH7VBPMCNjs.prisma.productReview.count({ where: { status: "APPROVED" } }),
    _chunkH7VBPMCNjs.prisma.productReview.count({ where: { status: "REJECTED" } }),
    _chunkH7VBPMCNjs.prisma.productReview.count({ where: { status: "FLAGGED" } }),
    _chunkH7VBPMCNjs.prisma.productReview.aggregate({
      where: { status: "APPROVED" },
      _avg: { rating: true }
    })
  ]);
...
async function canCustomerReviewProduct(productId, customerId, email) {
  ...
  if (customerId) {
    const purchase = await _chunkH7VBPMCNjs.prisma.orderItem.findFirst({
      where: {
        order: {
          customerId,
          paymentStatus: "PAID"
...
async function getOrdersForReviewRequest(daysAfterDelivery = 7, limit = 50) {
  ...
  const orders = await _chunkH7VBPMCNjs.prisma.order.findMany({
    where: {
      status: "DELIVERED",
      paymentStatus: "PAID",
...
async function bulkApproveReviews(reviewIds) {
  return _chunkH7VBPMCNjs.prisma.productReview.updateMany({
...
async function bulkDeleteReviews(reviewIds) {
  return _chunkH7VBPMCNjs.prisma.productReview.deleteMany({
    where: { id: { in: reviewIds } }
  });
}
...
async function getSeoConfig() {
  ...
  const settings = await _chunkH7VBPMCNjs.prisma.setting.findMany({
...
var getSiteSettings = _react.cache.call(void 0, async () => {
  try {
    const settings = await _chunkH7VBPMCNjs.prisma.siteSettings.findUnique({
...
async function getOrCreateSiteSettings() {
  ...
  if (!settings) {
    const created = await _chunkH7VBPMCNjs.prisma.siteSettings.create({
      data: {
...
async function updateSiteSettings(data) {
  ...
  const updated = await _chunkH7VBPMCNjs.prisma.siteSettings.upsert({
    where: { id: DEFAULT_SITE_SETTINGS_ID },
...
var EventBus = class {
  ...
  async triggerEventWorkflows(event) {
    try {
      const workflows = await _chunkH7VBPMCNjs.prisma.workflow.findMany({
        where: {
          enabled: true,
          triggerType: "EVENT"
        }
      ...
      for (const workflow of workflows) {
        ...
        const { executeWorkflow: executeWorkflow2 } = await Promise.resolve().then(() => _interopRequireWildcard(require("./engine-HIVHRTYO.js")));
...
async function getWorkflowTemplates(options) {
  ...
  const templates = await _chunkH7VBPMCNjs.prisma.workflowTemplate.findMany({
  ...
  if (includeWorkflowCount) {
    ...
    const workflowCounts = await _chunkH7VBPMCNjs.prisma.workflow.groupBy({
      by: ["templateId"],
...
async function getWorkflowTemplate(idOrSlug) {
  const template = await _chunkH7VBPMCNjs.prisma.workflowTemplate.findFirst({
    where: {
      OR: [
        { id: idOrSlug },
        { slug: idOrSlug }
  ...
  const workflowCount = await _chunkH7VBPMCNjs.prisma.workflow.count({
...
async function getRecommendedTemplates(limit = 4) {
  const installedTemplates = await _chunkH7VBPMCNjs.prisma.workflow.findMany({
...
async function installWorkflowTemplate(templateIdOrSlug, options) {
  ...
  try {
    ...
    let slug = baseSlug;
    let counter = 1;
    while (await _chunkH7VBPMCNjs.prisma.workflow.findUnique({ where: { slug } })) {
    ...
    const workflow = await _chunkH7VBPMCNjs.prisma.workflow.create({
      data: {
    ...
    const steps = await Promise.all(
      stepDefs.map(
        (step, index) => {
          var _a2, _b2, _c2;
          return _chunkH7VBPMCNjs.prisma.workflowStep.create({
            data: {
...
async function createTemplateFromWorkflow(workflowId, templateData) {
  ...
  return _chunkH7VBPMCNjs.prisma.workflowTemplate.create({
    data: {
...
async function updateTemplate(templateId, data) {
  const template = await _chunkH7VBPMCNjs.prisma.workflowTemplate.findUnique({
    where: { id: templateId }
  });
  if (!template) {
  ...
  return _chunkH7VBPMCNjs.prisma.workflowTemplate.update({
    where: { id: templateId },
    data
  });
}
async function deleteTemplate(templateId) {
  ...
  await _chunkH7VBPMCNjs.prisma.workflow.updateMany({
    where: { templateId },
    data: { templateId: null }
  });
  await _chunkH7VBPMCNjs.prisma.workflowTemplate.delete({
    where: { id: templateId }
  });
}
async function getTemplateStats(templateId) {
  ...
  const recentExecutions = await _chunkH7VBPMCNjs.prisma.workflowExecution.findMany({
    where: {
...
async function getCategoryStats() {
  const templates = await _chunkH7VBPMCNjs.prisma.workflowTemplate.groupBy({
    by: ["category"],
    _count: true
  });
  const workflows = await _chunkH7VBPMCNjs.prisma.workflow.findMany({
...
async function setWorkflowEnabled(workflowId, enabled) {
  try {
    ...
    const updatedWorkflow = await _chunkH7VBPMCNjs.prisma.workflow.update({
      where: { id: workflowId },
      data: { enabled }
    });
    if (enabled) {
...
async function disableAllWorkflows() {
  const enabledWorkflows = await _chunkH7VBPMCNjs.prisma.workflow.findMany({
    where: { enabled: true },
    select: { id: true }
  });
...
async function getEnabledWorkflowCounts() {
  const counts = await _chunkH7VBPMCNjs.prisma.workflow.groupBy({
    by: ["triggerType"],
    where: { enabled: true },
    _count: true
  });
...
async function initializeEventWorkflows() {
  const eventWorkflows = await _chunkH7VBPMCNjs.prisma.workflow.findMany({
    where: {
      enabled: true,
      triggerType: "EVENT"
    }
...
async function getShippingSettings() {
  ...
  try {
    ...
    if (settings.length > 0) {
      const settingsMap = {};
      for (const setting of settings) {
        settingsMap[setting.key] = setting.encrypted ? _chunkZNGPT5BQjs.safeDecrypt.call(void 0, setting.value) : setting.value;
      }
      const dbSettings = {
...
async function getDashboardConfig() {
  try {
    const setting = await _chunkH7VBPMCNjs.prisma.setting.findUnique({
...
async function saveDashboardConfig(config) {
  ...
  await _chunkH7VBPMCNjs.prisma.setting.upsert({
    where: { key: DASHBOARD_SETTINGS_KEY },
    create: {
...
var getRouteConfig = _react.cache.call(void 0, async (slug) => {
  ...
  const routeConfig = await _chunkH7VBPMCNjs.prisma.routeConfig.findUnique({
...
async function isSlugReserved(slug) {
  const normalizedSlug = slug.startsWith("/") ? slug : `/${slug}`;
  const existing = await _chunkH7VBPMCNjs.prisma.routeConfig.findUnique({
    where: { slug: normalizedSlug },
    select: { id: true }
  });
  return !!existing;
...
async function getAllRouteConfigs() {
  return _chunkH7VBPMCNjs.prisma.routeConfig.findMany({
    include: {
      page: {
        select: {
          id: true,
...
//# sourceMappingURL=chunk-CMUKQ2XI.js.map