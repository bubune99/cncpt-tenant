...
function setNodeOutput(context, nodeId, output) {
  return __spreadProps(__spreadValues({}, context), {
    nodeOutputs: __spreadProps(__spreadValues({}, context.nodeOutputs), {
      [nodeId]: output
    }),
...
function setVariable(context, name, value) {
  return __spreadProps(__spreadValues({}, context), {
    variables: __spreadProps(__spreadValues({}, context.variables), {
      [name]: value
    })
  });
}
...
function createIterationContext(context, itemVariable, item, indexVariable, index) {
  const variables = __spreadProps(__spreadValues({}, context.variables), {
    [itemVariable]: item
  });
  if (indexVariable) {
...
function serializeContext(context) {
  return JSON.stringify(__spreadProps(__spreadValues({}, context), {
    startedAt: context.startedAt.toISOString(),
    errors: context.errors.map((e) => __spreadProps(__spreadValues({}, e), {
...
function deserializeContext(serialized) {
  ...
  return __spreadProps(__spreadValues({}, parsed), {
    ...
    errors: parsed.errors.map((e) => __spreadProps(__spreadValues({}, e), {
...
async function getPrimitiveExecutionHistory(primitiveId, options) {
  return prisma.primitiveExecution.findMany({
    where: __spreadValues(__spreadValues({
      primitiveId
...
async function executeWorkflowInstance(workflow, options) {
  ...
  let context = createWorkflowContext(
    ...
    __spreadValues(__spreadValues({}, workflowVariables), options.variables)
  );
  context = __spreadProps(__spreadValues({}, context), { executionId: execution.id });
  const nodeResults = [];
  const startTime = Date.now();
  try {
...
async function executePrimitiveNode(node, config, context, workflowId) {
  ...
  const finalInputs = __spreadValues(__spreadValues({}, config == null ? void 0 : config.primitiveConfig), inputs);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (const edge of loopBodyEdges) {
      ...
      if (bodyNode) {
        ...
        currentContext = __spreadProps(__spreadValues({}, result.context), {
          variables: __spreadValues(__spreadProps(__spreadValues({}, result.context.variables), {
            [itemVariable]: void 0
...
//# sourceMappingURL=chunk-DZOH65BU.mjs.map