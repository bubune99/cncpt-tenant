...
async function recordEmailOpen(recipientId, metadata) {
  const now = /* @__PURE__ */ new Date();
  await _chunkI5PINI5Tjs.prisma.emailRecipient.update({
  ...
  if (metadata == null ? void 0 : metadata.campaignId) {
    await _chunkI5PINI5Tjs.prisma.emailCampaign.update({
  ...
  await _chunkI5PINI5Tjs.prisma.$executeRaw`
...
async function recordEmailClick(recipientId, url, metadata) {
  ...
  if (metadata == null ? void 0 : metadata.linkId) {
    await _chunkI5PINI5Tjs.prisma.emailLinkClick.create({
      data: {
    ...
    await _chunkI5PINI5Tjs.prisma.emailLink.update({
...
async function getOrCreateTrackedLink(campaignId, targetUrl) {
  let link = await _chunkI5PINI5Tjs.prisma.emailLink.findFirst({
    where: {
      campaignId,
      url: targetUrl
    }
  ...
  if (!link) {
    link = await _chunkI5PINI5Tjs.prisma.emailLink.create({
      data: {
        campaignId,
        url: targetUrl,
        clickCount: 0
...
async function handleBounce(event) {
  ...
  await _chunkI5PINI5Tjs.prisma.emailSubscriber.updateMany({
  ...
  if (event.recipientId) {
    ...
    await _chunkI5PINI5Tjs.prisma.emailRecipient.updateMany({
      where: {
        email: event.email,
...
async function subscribeEmail(email, options = {}) {
  ...
  try {
    const existing = await _chunkI5PINI5Tjs.prisma.emailSubscriber.findUnique({
    ...
    if (existing) {
      ...
      if (["UNSUBSCRIBED", "CLEANED"].includes(existing.status)) {
        ...
        const updated = await _chunkI5PINI5Tjs.prisma.emailSubscriber.update({
          where: { email: normalizedEmail },
          data: {
    ...
    const subscriber = await _chunkI5PINI5Tjs.prisma.emailSubscriber.create({
      data: {
...
async function confirmSubscription(token) {
  try {
    const subscriber = await _chunkI5PINI5Tjs.prisma.emailSubscriber.findUnique({
...
async function unsubscribeEmail(email, options = {}) {
  ...
  try {
    ...
    await _chunkI5PINI5Tjs.prisma.emailSubscriber.update({
      where: { email: normalizedEmail },
      data: {
        status: "UNSUBSCRIBED",
        unsubscribedAt: /* @__PURE__ */ new Date(),
...
async function getSubscriberPreferences(subscriberIdOrEmail) {
  try {
    let subscriber = await _chunkI5PINI5Tjs.prisma.emailSubscriber.findUnique({
      where: { id: subscriberIdOrEmail }
    });
    if (!subscriber) {
      subscriber = await _chunkI5PINI5Tjs.prisma.emailSubscriber.findUnique({
...
async function sendConfirmationEmail(email, token) {
  const { sendEmail: sendEmail2 } = await Promise.resolve().then(() => _interopRequireWildcard(require("./email-DPBLNZ2C.js")));
...
var EmailQueue = class {
  ...
  async persistEmail(email) {
    try {
      await _chunkI5PINI5Tjs.prisma.emailQueueItem.create({
        data: {
          id: email.id,
  ...
  async updatePersistedEmail(email) {
    try {
      await _chunkI5PINI5Tjs.prisma.emailQueueItem.update({
        where: { id: email.id },
        data: {
          status: email.status,
          attempts: email.attempts,
  ...
  async loadFromDatabase() {
    ...
    try {
      const items = await _chunkI5PINI5Tjs.prisma.emailQueueItem.findMany({
        where: {
...
//# sourceMappingURL=chunk-MT3LB7M4.js.map